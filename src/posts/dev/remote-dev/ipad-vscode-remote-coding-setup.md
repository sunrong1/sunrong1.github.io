...
## 补充：服务器升级与 AI 编程工具链（2026-06-19）

### 服务器升级（618 活动）

顺手升级了云服务器配置，提升远程开发体验：
...
**升级理由**：
- 4 核 CPU 可以同时跑 code-server + 2~3 个 Agent 进程不卡顿
- 内存保持 4 GB 不变（code-server 本身内存占用不高）
- 618 活动价格对比平时便宜 30~40%，年付最划算

### 网络专线：了解后暂不使用

考察了 BGP 网络专线 / CN2 GIA 等高阶网络方案：

- ✅ 优点：带宽稳定、丢包率低、延迟低（10ms 以内）
- ❌ 缺点：价格是普通带宽的 5~10 倍（年费 2000~5000 元）
- 🎯 决策：**当前开发场景（个人远程 + Agent 测试）对延迟不敏感，普通带宽 + CDN 已足够**
...
| 特性 | 说明 |
|------|------|
| **GLM 5.2 模型支持** | 智谱最新模型，代码能力接近 GPT-4 级别 |
| **免费额度** | 注册即可白嫖日常使用量，个人开发足够 |
| **VSCode 插件** | 跟 Cursor/Continue 类似，能在编辑器里直接用 |
| **中文友好** | 提示词 / 文档 / 错误信息都是中文优先 |

**与 code-server 协同工作流**：

```
iPad (Edge) → code-server (远程容器) → Code Buddy AI 助手
                                          ↓
                                    GLM 5.2 模型生成代码
```

**实测场景**：在 code-server 里写代码时，让 Code Buddy 解释一段 AgentScope 源码的意图，回复准确度 90%+，完全够用。

**推荐指数**：⭐⭐⭐⭐（个人开发免费 + 模型强 + 中文友好）
...
└────────┬───────┘
         ↓
┌────────────────┐
│ code-server    │ ← 远程容器（4核4g）
│  + Code Buddy  │ ← AI 编程助手（GLM 5.2）
│  + LLM API     │ ← 多模型切换
└────────┬───────┘
         ↓ gh-proxy.com
┌────────────────┐
...
| 投入 | 月成本 |
|------|--------|
| 云服务器（4核4g） | ~30 元/月（年付折算）|
| 域名（hermes.sunrong.site） | ~10 元/月（年付折算）|
| Code Buddy | 0 元（白嫖）|
| **合计** | **~40 元/月** |
...
通勤 2h **不再是单一动作**，而是 **双轨输入**：
- **代码输入**（约 1h）：通勤路上用 code-server 看 AI Agent 框架源码（AgentScope / Spring AI MCP / LangChain）
- **微信读书**（约 1h）：本周在读 **《西蒙学习方法》** 和 **孙宇晨《这个世界既残酷又温柔》**

### 3. 真实的工作流（几个月稳定执行）

| 时段 | 动作 | 工具 | 性质 |
|------|------|------|------|
| 通勤（地铁 2h）| 读代码 + 微信读书 | 14 寸小本 + code-server + 微信读书 App | **输入** |
| 白天 | 写代码 / 业务 | 工位电脑 | **输出** |
| 晚上 | Vibe Coding 实战 / Blog | 任何设备 | **输出 + 复盘** |
| 周末 | 总结 / 陪 Max | 家庭时间 | **复盘 + 陪伴** |

### 4. 关键洞察

> **通勤读代码 vs 写代码 = 输入 vs 输出 = 物理环境与认知活动必须匹配**

地铁晃 → **不适合写代码**（需要稳态输出）| **适合读代码 + 读书**（被动接收）
工位稳 → **适合写代码**（业务交付）| **适合实战**（Vibe Coding）

**这是 v5 最舒服状态的真实物质化**：**几个月稳定执行 = 找到最适合自己的工作流 = 真正的复利曲线在跑了**。
