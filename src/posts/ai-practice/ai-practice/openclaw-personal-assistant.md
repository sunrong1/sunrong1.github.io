...
| 12 | 版本自动检查导致 CPU 占满 | 锁死版本 `openclaw update --tag <version> --no-restart` |

---

### 🐞 坑13：版本自动检查导致 CPU 占用异常

**问题现象**：OpenClaw 突然变得特别卡顿，Claude Code 检查多次都正常，怀疑配置或上下文问题，最终用 `top` 命令发现有一个异常的 `npm install` 进程占用大量 CPU。

**发生时间**：2026-04-29

**根本原因**：OpenClaw 每次启动时会通过 npm 检查最新版本（当前 2026.4.24，最新 2026.4.26），触发下载和编译，导致 CPU 占用飙升。不是 OpenClaw 本身的 bug，是版本检查机制导致的。

**排查过程**（标准思路）：
1. 怀疑配置错误 → Claude Code 检查多次，正常
2. 怀疑上下文过长 → 压缩后依然卡顿
3. 怀疑网络问题 → 不是
4. **用 `top` 命令查看进程** → 发现 `npm install` 占用 CPU
5. 问 DeepSeek → 确认是 OpenClaw 自动更新检查
...
**效果**：✅ 版本锁死，不再自动检查更新，CPU 占用恢复正常

**后续如何更新**：
- 想升版本：`openclaw update --tag 2026.4.26 --yes`
- 保持锁死：不需要任何操作

**经验总结**：遇到莫名卡顿，先 `top` 看进程状态，是最直接的排查方式。
